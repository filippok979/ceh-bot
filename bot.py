...
def copy_to_admin(original_message, menu_type):
    ...
    # Получаем номер телефона из контакта или устанавливаем "не указан"
    phone_number = "не указан"
    if 'contact' in original_message:
        phone_number = original_message['contact'].get('phone_number', 'не указан')
    ...
    # Информационное сообщение с HTML разметкой
    info_message = (
        f"📞 🚨 {menu_type} 🚨\n"
    ...
    send_message(ADMIN_CHAT_ID, info_message, parse_mode='HTML')
...
def webhook():
    ...
    if 'message' in update:
        ...
        if 'contact' in message:
            phone_number = message['contact'].get('phone_number', 'не указан')
        ...
        if text == '/start':
            ...
            welcome_text = (
                "🔒 <b>ВАЖНО: Проверьте настройки приватности!</b>\n"
                "📩 Чтобы мастер мог оперативно связаться с вами, убедитесь, что в настройках Telegram разрешены сообщения от всех пользователей:\n"
                "⚙️ Настройки → Конфиденциальность → Личные сообщения → «Все»\n\n"
                "📢 <b>Подпишитесь на наш канал:</b>\n"
                "👉 https://t.me/mebel_136\n\n"
                "👋 Добро пожаловать!\n\n"
                "🎯 Если не любите много читать — сразу переходите в меню ниже! 😊\n\n"
                "💡 А для тех, кто хочет сделать заказ мебели максимально комфортным, мы подготовили 3 важных совета:\n\n"
                "📋 3 ключевых шага при заказе мебели:\n\n"
                "1. 💰 Четко определите и озвучьте бюджет\n"
                "Просто сообщите нам сумму, которую планируете потратить. Это поможет сразу предложить оптимальные решения без нереалистичных вариантов и сэкономит ваше время.\n"
                "*Например: \"Мой бюджет на шкаф-купе — 80 000 рублей с учетом сборки\".*\n\n"
                "2. 🎨 Изучите стиль заранее\n"
                "Сохраните 3-5 фото понравившейся мебели из интернета. Это поможет нам быстрее понять ваши предпочтения и точнее оценить стоимость.\n"
                "*Учтите: классика с резными фасадами из массива обычно на 30-50% дороже лаконичного минимализма.*\n\n"
                "3. 🤝 Доверяйте экспертам, а не инфоцыганам\n"
                "Мы всегда предлагаем разумные альтернативы:\n"
                "• 🔍 Объясним, почему массив дуба на кухне непрактичен (остаются следы от воды, требует регулярного обновления покрытия)\n"
                "• 🚪 Посоветуем дорогие подъемные механизмы только для ежедневно используемых шкафов\n"
                "💫 Такой подход экономит ваш бюджет без потери качества.\n\n"
                "🚀 Готовы создать мебель вашей мечты? Воспользуйтесь меню ниже, чтобы начать!"
            )
            
            send_message(chat_id, welcome_text, main_keyboard, parse_mode='HTML')
            
            # Отправляем сообщение в админ чат о новом пользователе
            user_link = f"<a href='tg://user?id={user_id}'>{user_name}</a>"
            admin_start_message = (
                f"🆕 Новый пользователь начал работу с ботом\n"
                f"👤 Пользователь: {user_link}\n"
                f"🆔 ID: <code>{user_id}</code>\n"
                f"📞 Телефон: <code>{phone_number}</code>\n"
                f"⏰ Время: {datetime.now().strftime('%H:%M %d.%m.%Y')}"
            )
            send_message(ADMIN_CHAT_ID, admin_start_message, parse_mode='HTML')
